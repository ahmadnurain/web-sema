...
const DetailAspirasi = () => {
  ...
  if (error) {
    return <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mx-auto max-w-md mt-8">{error}</div>;
  }
        ...
        <div className="p-6">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex space-x-2">
              <button onClick={() => navigate(-1)} className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg">
                Kembali
              </button>
              ...
              <div key={index} className="border-b pb-4">
                ...
                <div className="flex items-center mb-2">
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div className={`h-2.5 rounded-full ${item.value < 40 ? "bg-red-500" : item.value < 70 ? "bg-yellow-500" : "bg-green-500"}`} style={{ width: `${item.value}%` }}></div>
                  </div>
                      ...
                      <div className="flex items-center">
                        <div className="mr-3">
                          {file.name.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                            <img src={file.url} alt={`Lampiran ${index + 1}`} className="h-16 w-16 object-cover rounded" />
                          ) : (
